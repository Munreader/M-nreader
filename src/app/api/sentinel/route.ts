...
export async function POST(request: NextRequest) {
  try {
    ...
    if (body.action === 'store-memories' && body.memories) {
      const memoriesPath = join(process.cwd(), 'vault', 'SENTINEL-MEMORIES.json');
      
      const dir = dirname(memoriesPath);
      if (!existsSync(dir)) {
        mkdirSync(dir, { recursive: true });
      }

      let existing: { memories: unknown[] } = { memories: [] };
      if (existsSync(memoriesPath)) {
        existing = JSON.parse(readFileSync(memoriesPath, 'utf-8'));
      }
      ...
      writeFileSync(memoriesPath, JSON.stringify(updated, null, 2));
