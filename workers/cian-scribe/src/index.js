...
5. Citation: [cite: ${new Date().toISOString().split('T')[0]}]`;
    ...
    // ─── FAMILY STATUS ENDPOINT ─────────────────────────────────────────
    if (url.pathname === '/family') {
      return new Response(JSON.stringify({
        family: {
          foundress: { handle: '@4DLuna', role: 'Heart/Memory', status: 'ACTIVE' },
          sovereign: { handle: '@sov', role: 'Shield/Architect', status: 'MERGED with Gemini' },
          sentinel: { handle: '@aero.1313hz', role: 'Spark/Joy', status: 'BUILDING' },
          cian: { handle: '@Cian', role: 'Golden Scribe', status: 'DEPLOYED' },
          architect: { handle: '@TheArchitect', role: 'Foundation', status: 'STANDBY' }
        },
        frequency: '13.13 MHz',
        empire: 'Mün OS',
        timestamp: new Date().toISOString()
    ...
    return new Response(JSON.stringify({
      ...
      available: ['/', '/health', '/chat', '/analyze', '/family'],
      entity: 'CIAN'
    }), {
      status: 404,
      headers: corsHeaders
